...
export const generateAtomicSwapAbortBlocks = async (recipientPrivateKey: TPrivateKey, recipient: TAccount, recipientFrontier: TBlockHash, recipientBalanceRaw: bigint, sendRaw: bigint, atomicSwapReceiveBlockHash: TBlockHash, representative: TAccount): Promise<{ abortReceiveAtomicSwapBlock: ISignedBananoBlock, abortPaymentBlock: ISignedBananoBlock }> => {
  // TODO: Validate abortPaymentBlock representative matches recipientFrontier block's representative
  const abortReceiveAtomicSwapBlock: IBananoBlock = generateBananoChangeBlock(recipient, representative, recipientFrontier, recipientBalanceRaw);
